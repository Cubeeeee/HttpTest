...
namespace 参数化Http请求
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            manager = InitUpdateManager(Exe_Path, Xml_Path, Root_Path, Root_Id);
        }
        UpdateManager InitUpdateManager(String Exe_Path, String Xml_Path, String Root_Path, String RootId)
        {
            UpdateManager manager = new UpdateManager();
            if (File.Exists(Xml_Path))
            {
                manager = new UpdateManager(Xml_Path, Path.GetFileName(Exe_Path), Root_Path, false);
            }
            else
            {
                manager = new UpdateManager(Xml_Path, Path.GetFileName(Exe_Path), Root_Path, true);
                manager.Rootid = RootId;
                manager.Configer.AddValue("RootId", RootId);
            }
